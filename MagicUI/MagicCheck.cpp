...
#include "stdafx.h"
#include "Include/MagicUI/MagicCheck.h"
...
IMPLEMENT_DYNAMIC(CMagicCheck,CButton);

CMagicCheck::CMagicCheck()
{
...
BEGIN_MESSAGE_MAP(CMagicCheck, CButton)
	//{{AFX_MSG_MAP(CMagicCheck)
...
/////////////////////////////////////////////////////////////////////////////
// CMagicCheck message handlers

//创建按钮
BOOL CMagicCheck::Create(LPCTSTR lpszCaption, DWORD dwStyle,
...
//自动加载
BOOL CMagicCheck::AutoLoad(UINT nID, CWnd* pParent)
{
	m_nID=nID;
	m_pWnd=pParent;
...
int CMagicCheck::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//重载DrawItem函数
void CMagicCheck::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
void CMagicCheck::OnMouseMove(UINT nFlags, CPoint point) 
{
...
LRESULT CMagicCheck::OnMouseLeave(WPARAM wParam,LPARAM lParam)   
{
	m_bMouseOn=FALSE;
	Invalidate();
	return TRUE;
...
void CMagicCheck::OnLButtonUp(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	CRect rect;
	GetClientRect(&rect);
...
BOOL CMagicCheck::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	// TODO: Add your message handler code here and/or call default
	if (m_bMouseOn)
	{
...
void CMagicCheck::SetForeColor(COLORREF foreColor)
{
...
void CMagicCheck::SetForeColor(COLORREF foreColor,COLORREF foreColorFocus,
...
void CMagicCheck::SetBitmap(CBitmap &bitmap)
{
	if(bitmap.m_hObject)
	{
...
void CMagicCheck::SetBitmap(UINT nResID)
{
	if(m_backgndFull.m_hObject)
		m_backgndFull.DeleteObject();
...
void CMagicCheck::SetBitmap(LPCTSTR lpszFileName)
{
...
void CMagicCheck::SetBitmap(HBITMAP hBitmap)
{
	if(hBitmap)
	{
		if(m_backgndFull.m_hObject)
...
void CMagicCheck::SetCursor(HCURSOR hCursor)
{
	m_hCursor=hCursor;
	Invalidate();
}
...
//设置字体
void CMagicCheck::SetFont(int nFontSize,CString strFontName,BOOL bBold,BOOL bItalic,
...
//获取要创建区域的父窗口背景
void CMagicCheck::GetBackgndMask()
{
...
//绘透明位图
void CMagicCheck::DrawTransparentBitmap(CDC *pDC,CRect rect,CBitmap &foreBmp,COLORREF maskColor)
{ 
...
void CMagicCheck::SetMaskColor(COLORREF color)
{
	m_maskColor=color;
}

//绘普通位图
void CMagicCheck::DrawOriginalBitmap(CDC *pDC, CRect rect,CBitmap &foreBmp)
{
	CDC dc;
...
//绘GDI+图
void CMagicCheck::DrawImage(CDC *pDC,CRect rect,LPCTSTR lpszFileName,int nIndex)
{
	USES_CONVERSION;
...
//GDI+背景图
void CMagicCheck::SetImage(LPCTSTR lpszFileName)
{
...
BOOL CMagicCheck::OnEraseBkgnd(CDC* pDC)
{
	if(!m_backgndMask.m_hObject)
		GetBackgndMask();
	return TRUE;
...
//对齐方式
void CMagicCheck::SetAlign(UINT nAlign)
{
	m_nAlign=nAlign;
	Invalidate();
}
...
//设置选中状态
void CMagicCheck::SetSelect(BOOL bSelect)
{
	m_bSelected=bSelect;
	Invalidate();
}
...
//获取选中状态
BOOL CMagicCheck::GetSelect()
{
	return m_bSelected;
}

BOOL CMagicCheck::EnableWindow(BOOL bEnable)
{
	m_bEnable=bEnable;
	return CButton::EnableWindow(bEnable);
}

BOOL CMagicCheck::GetEnable()
{
	return m_bEnable;
}

BOOL CMagicCheck::ShowWindow(int nCmdShow)
{
	if(nCmdShow==SW_HIDE)
	{
		m_bVisible=FALSE;
